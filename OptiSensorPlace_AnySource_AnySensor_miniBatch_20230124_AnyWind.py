...
Num_Backtracking = 7  # for adapting the outer iteration step size (or called learning rate)
lr_outer_initial = 0.00025  # for the initial learning rate of the backtracking
...
def GradientOuterNew(x_all, y_all, source_x, source_y, w_x, w_y, u, K_tem, H_tem, Phi_tem, sigma_e, lambda_1_tem,
    ...
    for mk in range(num_sensor):
        for i in range(num_source):
            for j in range(num_source):
                G_x[mk, i] += 1. / (sigma_e ** 2) * Gradient_AA_x(x_all[mk], y_all[mk], source_x[i], source_y[i],
                                                                  source_x[j], source_y[j],
                                                                  w_x, w_y, u, K_tem[i], H_tem[i], K_tem[j], H_tem[j]) * \
                              theta_curr[j]
                G_y[mk, i] += 1. / (sigma_e ** 2) * Gradient_AA_y(x_all[mk], y_all[mk], source_x[i], source_y[i],
                                                                  source_x[j], source_y[j],
                                                                  w_x, w_y, u, K_tem[i], H_tem[i], K_tem[j], H_tem[j]) * \
                              theta_curr[j]
            G_x[mk, i] -= 1. / (sigma_e ** 2) * Gradient_A_x(x_all[mk], y_all[mk], source_x[i], source_y[i], w_x, w_y,
                                                             u, K_tem[i],
                                                             H_tem[i]) * Phi_tem[mk]
            G_y[mk, i] -= 1. / (sigma_e ** 2) * Gradient_A_y(x_all[mk], y_all[mk], source_x[i], source_y[i], w_x, w_y,
                                                             u, K_tem[i],
                                                             H_tem[i]) * Phi_tem[mk]
    # print('G_x:', G_x)
...
for k in range(n_k):
    ...
    for start_batch in range(0, N_samples, batch_size):
        ...
        if k > 0:
            ...
            while (Theta_error_norm_step_k[k * num_batch + count_temp] - Theta_error_norm_step_k[
                ...
                temp_Gx[nmp.isnan(temp_Gx)] = 0  # to avoid abnormal gradients
                temp_Gy[nmp.isnan(temp_Gy)] = 0  # to avoid abnormal gradients
                # update
    ...
    if (nmp.all(nmp.abs(nmp.array(temp_Gx)) < tol_G) and nmp.all(nmp.abs(nmp.array(temp_Gy)) < tol_G)) and k > 1:
        if nmp.all(0 < nmp.abs(nmp.array(temp_Gx))) and nmp.all(0 < nmp.abs(nmp.array(temp_Gy))):
